...
export default function View({ slug: fromServer }: { slug: string[] }) {
  ...
  const [contractId, docType, indexName, indexValuesEncoded] = slug ?? [];
  const indexValues = indexValuesEncoded ? parseIndexValues(indexValuesEncoded) : null;
  ...
  const q = useContestedResourceVoteState(contractId, docType, indexName, indexValues ?? undefined);
  ...
  if (!contractId || !docType || !indexName || !indexValues) {
    return (
      <Container py={8}>
        <InfoBlock>
          <Text color="gray.250">
            Malformed contested-resource URL. Expected
            <code>/governance/contested/&lt;contractId&gt;/&lt;docType&gt;/&lt;indexName&gt;/&lt;indexValuesJSON&gt;/</code>.
          </Text>
        </InfoBlock>
      </Container>
    );
  ...
  const data = q.data;
  const contenders =
    (readProp<unknown[]>(data, 'contenders') as unknown[] | undefined) ?? [];
          ...
          <VStack align="flex-start" spacing={3}>
            ...
            <HStack spacing={6} flexWrap="wrap">
              <InfoLine
                label="Contract"
                value={
                  <Identifier value={contractId} href={`/contract/${contractId}/`} dense avatar={false} />
                }
              />
              ...
              <InfoLine
                label="Values"
                value={<Text fontFamily="mono">{indexValues.map((v) => String(v)).join(' / ')}</Text>}
              />
            </HStack>
          </VStack>
        </InfoBlock>
        ...
        {q.isLoading ? (
          ...
          <InfoBlock>
            <Text color="gray.400" fontSize="sm">
              No vote state returned. The contest may be closed or the index values may not match.
            </Text>
          </InfoBlock>
        ) : (
          <>
