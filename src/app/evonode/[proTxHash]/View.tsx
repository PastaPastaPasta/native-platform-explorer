...
export default function View({ proTxHash: fromServer }: { proTxHash: string }) {
  ...
  const blocksQ = useEvonodesBlocksByIds(
    proTxHash !== 'placeholder' ? currentIdx : undefined,
    proTxHash !== 'placeholder' ? [proTxHash] : undefined,
  );
  const voteQ = useProtocolVersionUpgradeVoteStatus(undefined, 100);

  if (proTxHash === 'placeholder') {
    return (
      <Container py={8}>
        <InfoBlock>
          <Text color="gray.250">Provide an evonode proTxHash in the URL.</Text>
        </InfoBlock>
      </Container>
    );
  }
