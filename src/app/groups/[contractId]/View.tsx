...
export default function View({ contractId: fromServer }: { contractId: string }) {
  ...
  const q = useGroupInfos(contractId);

  const groups = (() => {
    const raw = q.data;
    if (!raw) return [];
    if (raw instanceof Map) {
      return [...raw.entries()].map(([pos, info]) => ({
        position: Number(pos),
        info,
      }));
    }
  ...
  if (contractId === 'placeholder') {
    return (
      <Container py={8}>
        <InfoBlock>
          <Text color="gray.250">Provide a real contract id in the URL.</Text>
        </InfoBlock>
      </Container>
    );
  }
      ...
      <VStack align="stretch" spacing={4}>
        <InfoBlock emphasised>
          <VStack align="flex-start" spacing={2}>
            ...
            <Identifier value={contractId} href={`/contract/${contractId}/`} dense />
          </VStack>
        </InfoBlock>
                    ...
                    return (
                      <Tr key={g.position} _hover={{ bg: 'gray.800' }}>
                        ...
                        <Td borderColor="gray.750" textAlign="right">
                          <Button
                            as={NextLink}
                            href={`/groups/${contractId}/${g.position}/`}
                            size="xs"
                            variant="outline"
                            colorScheme="blue"
                          >
