...
function contestedLink(label: string): string | null {
  ...
  return (
    `/governance/contested/detail/?contract=${encodeURIComponent(dpns.testnetId)}` +
    `&docType=domain&indexName=parentNameAndLabel&indexValues=${encodeURIComponent(values)}`
  );
}
...
function Content() {
  ...
  const name = rawName ? normaliseDpnsName(decodeURIComponent(rawName)) : '';
  ...
  const resolveQ = useDpnsResolve(name || undefined);
  const recordQ = useDpnsGetByName(name || undefined);
  const availableQ = useDpnsIsAvailable(name || undefined);
  const contestedQ = useDpnsIsContested(name || undefined);
  const validQ = useDpnsIsValid(name || undefined);
  ...
  if (!name) {
    return (
      <Container py={8}>
        <InfoBlock>
          <Text color="gray.250">
            Provide a DPNS name as <code>?name=…</code>, e.g.{' '}
            <code>/dpns/?name=alice.dash</code>.
          </Text>
        </InfoBlock>
      </Container>
    );
